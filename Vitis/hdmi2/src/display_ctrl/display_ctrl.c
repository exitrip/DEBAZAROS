...
//	ClkFindParams(_dynclk_mm_IP, dispPtr->vMode.freq, &clkMode);
	...
	/*
	 * Use precalculated MMCM confs
	 */
	clkMode.freq = dispPtr->vMode.clkConf.freq;
	clkMode.fbmult = dispPtr->vMode.clkConf.fbmult;
	clkMode.clkdiv = dispPtr->vMode.clkConf.clkdiv;
	clkMode.maindiv = dispPtr->vMode.clkConf.maindiv;
